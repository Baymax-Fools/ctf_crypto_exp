...
p = remote("node1.anna.nssctf.cn",23887)
...
m1 = b'1'
p.sendlineafter(b'sign: \n', m1)

p.recvuntil(b's = ')
s1 = int(p.recvline().strip().decode())
p.recvuntil(b't = ')
t1 = int(p.recvline().strip().decode())
s1 = Hinv(s1,k)
t1 = Hinv(t1,k)
...
m2 = literal_eval(p.recvline().strip().decode())
...
p.sendline((str(s2) + ',' + str(t2)).encode())
